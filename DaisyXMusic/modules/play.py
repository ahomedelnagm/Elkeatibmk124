...
async def ee(client, message):
    ...
    if stats:
        ...
        await message.reply("لا توجد مكالمات قيد التشغيل في هذه الدردشة")
...
async def play(_, message: Message):
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    await lel.edit(
                        "✅ ** انضم userbot بنجاح لهذه المجموعة **",
                    )
    ...
    if audio:
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("القائمه", callback_data="playlist"),
                    InlineKeyboardButton("التشغيل", callback_data="menu"),
                ],
        ...
        try:
            ...
            if (dur / 60) > DURATION_LIMIT:
                await lel.edit(
                    f"❌ مقاطع الفيديو أطول من {DURATION_LIMIT} الدقائق غير مسموح لها باللعب!"
                )
                return
        except:
            pass
        ...
        keyboard = InlineKeyboardMarkup(
            [
                ...
                [
                    InlineKeyboardButton(text="يوتيوب", url=f"{url}"),
                    InlineKeyboardButton(text="- ♫︎༆𝙈𝙐𝙎𝙄𝘾_𝙀𝙇𝙆𝙀𝘼𝙏𝙄𝘽༆♫︎ ›",url=f"https://t.me/MusicElkeatib")
                ],
                [InlineKeyboardButton(text="الغاء", callback_data="cls")],
            ]
        )
        ...
        try:
            ...
            await lel.edit("لا توجد نتائج كافية للاختيار .. بدء اللعب المباشر")
    ...
    if chat_id in callsmusic.active_chats:
        ...
        await message.reply_photo(
            photo="final.png",
            reply_markup=keyboard,
            caption="تم تشغيل الاغنيه بواسطة {} رايق صح 🎉.".format(
...
async def ytplay(_, message: Message):
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                ...
                try:
                    await USER.join_chat(invitelink)
                    await USER.send_message(
                        message.chat.id, "انضممت إلى هذه المجموعة لتشغيل الموسيقى في المكالمه"
                    )
                    await lel.edit(
                        "<b>انضم الحساب المساعد إلي محادثتك</b>",
                    )
    ...
    try:
        ...
        await lel.edit("لم يتم العثور على الأغنية ، جرب أغنية أخرى أو ربما تهجئها بشكل صحيح.")
        print(str(e))
        return
    try:
        secmul, dur, dur_arr = 1, 0, duration.split(":")
...
async def jiosaavn(client: Client, message_: Message):
        ...
        for administrator in administrators:
            if administrator == message_.from_user.id:
                ...
                try:
                    await USER.join_chat(invitelink)
                    await USER.send_message(
                        message_.chat.id, "🦹🏻: لقد انضممت إلى هذه المجموعة لتشغيل الموسيقى في الدردشة الصوتية"
                    )
                    await lel.edit(
                        "✅ ** انضم userbot بنجاح إلى هذه المجموعة.**",
                    )
    ...
    try:
        ...
        await lel.edit(
            "<i> المساعد ليس في هذه الدردشة ، اطلب من المسؤول الإرسال /play الأمر لأول مرة أو إضافة مساعد يدويًا</i>"
        )
        return
    ...
    keyboard = InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton(
                    text="- ♫︎༆𝙈𝙐𝙎𝙄𝘾_𝙀𝙇𝙆𝙀𝘼𝙏𝙄𝘽༆♫︎ ›", url=f"https://t.me/{updateschannel}"
                )
            ],
            [InlineKeyboardButton(text="الغاء", callback_data="cls")],
        ]
...
async def lol_cb(b, cb):
    ...
    if cb.from_user.id != useer_id:
        await cb.answer(
            "أنت لست الشخص الذي طلب تشغيل الأغنية!", show_alert=True
        )
        return
    await cb.message.edit("**جاري التحميل ...🎉**")
    x = int(x)
    try:
    ...
    try:
        ...
        if (dur / 60) > DURATION_LIMIT:
            await cb.message.edit(
                f"الموسيقى أطول من {DURATION_LIMIT}دقيقة غير مسموح لها باللعب"
            )
            return
    except:
        pass
    ...
    if chat_id in callsmusic.active_chats:
        ...
        await b.send_photo(
            ...
            photo="final.png",
            reply_markup=keyboard,
            caption=f"تم تشغيل الاغنيه بواسطة {r_by.mention()} رايق صح 🎉.",
        )
        os.remove("final.png")
